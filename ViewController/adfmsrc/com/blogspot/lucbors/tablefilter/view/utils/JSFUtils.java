package com.blogspot.lucbors.tablefilter.view.utils;
...
public class JSFUtils {
    ...
    public static String getStringFromBundle(String key, String bundle) {
        ResourceBundle temp = getFacesContext().getApplication().getResourceBundle(getFacesContext(), bundle);
        return getStringSafely(temp, key, null);
    }
    ...
    public static FacesMessage getMessageFromBundle(String key, FacesMessage.Severity severity, String bundle) {
        ResourceBundle temp = getFacesContext().getApplication().getResourceBundle(getFacesContext(), bundle);
        String summary = getStringSafely(temp, key, null);
        //      String detail = getStringSafely(temp, key + "_detail", summary);
        FacesMessage message = new FacesMessage(summary, summary);
        message.setSeverity(severity);
        return message;
    }
   ...
   */
    ...
    public static void logout(String redirectUrl) throws IOException {
        FacesContext fctx = FacesContext.getCurrentInstance();
        ExternalContext ectx = fctx.getExternalContext();
        //        HttpSession session = (HttpSession)ectx.getSession(false);
        //        session.invalidate();

        // TODO redirect is needed
        /*
       * URL for DEV : ectx.getRequestContextPath()
       * URL for ITT : ectx.getRequestContextPath()
       * URL for ACC : "https://testsa.b-rail.be/dana-na/auth/url_18/logout.cgi"
       */
        String urlPrefix = ectx.getRequestContextPath() + "/adfAuthentication?logout=true&end_url=";

        if (redirectUrl == null || redirectUrl.isEmpty()) {
            ectx.redirect(urlPrefix + ectx.getRequestContextPath());
        } else {
            ectx.redirect(urlPrefix + redirectUrl);
        }
